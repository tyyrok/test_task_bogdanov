...
def get_url() -> str:
    ...
    if app_settings.ENVIRONMENT in ["production"]:
        return f"postgresql://{postgres_user}:{postgres_password}@{postgres_server}:{postgres_port}/{postgres_db}?sslmode=verify-full&target_session_attrs=read-write&sslrootcert={os.path.join(app_settings.BASE_DIR, 'root.crt')}"
